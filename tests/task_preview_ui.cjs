...
(async () => {
  ...
  const profile = await run("api('/api/accounts/1/profile')");
  assert.equal(profile.first_name, 'Анна');
  await run("api('/api/accounts/1/profile', {method:'PATCH', body:JSON.stringify({about:'New bio'})})");
  assert.equal((await run("api('/api/accounts/1/profile')")).about, 'New bio');
