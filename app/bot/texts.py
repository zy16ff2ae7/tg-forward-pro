...
def welcome(name: str) -> str:
    ...
    # Пробный период выдаётся сам при первом заходе (см. ensure_user): это
    # первое, что человеку стоит знать, — всё уже включено и бесплатно.
    trial_line = (
        f"\n\n🎁 <b>Пробный период — {settings.trial_days} дн. бесплатно</b>: "
        "все задачи включены сразу, карта не нужна."
        if settings.trial_days > 0
        else ""
    )
    # Подарок за подписку — следом: плюс дни к абонементу.
    bonus_line = (
        f"\n\n➕ <b>Ещё {settings.bonus_days} дн. бесплатно</b> за подписку на "
